package hse.sheduleteachers.Calendar.utils;
...
public final class CalendarUtils {
    ...
    public static Month createMonth(Date date, SettingsManager settingsManager) {
        ...
        final Date firstDayOfMonth = hse.sheduleteachers.Calendar.utils.DateUtils.getFirstDayOfMonth(date);
        ...
        final Date firstDisplayedDay = hse.sheduleteachers.Calendar.utils.DateUtils.getFirstDayOfWeek(firstDayOfMonth, settingsManager.getFirstDayOfWeek());
        firstDisplayedDayCalendar.setTime(firstDisplayedDay);

        final Calendar end = Calendar.getInstance();
        end.setTime(hse.sheduleteachers.Calendar.utils.DateUtils.getLastDayOfWeek(hse.sheduleteachers.Calendar.utils.DateUtils.getLastDayOfMonth(date)));
        ...
        do {
            hse.sheduleteachers.Calendar.utils.DateUtils.addDay(firstDisplayedDayCalendar);
            days.add(createDay(firstDisplayedDayCalendar, settingsManager, targetMonth));
        } while (!hse.sheduleteachers.Calendar.utils.DateUtils.isSameDayOfMonth(firstDisplayedDayCalendar, end)
                || !hse.sheduleteachers.Calendar.utils.DateUtils.isSameMonth(firstDisplayedDayCalendar, end));
    ...
    private static List<DayOfWeek> createDaysOfWeek(Date firstDisplayedDay) {
        ...
        final Calendar calendar = hse.sheduleteachers.Calendar.utils.DateUtils.getCalendar(firstDisplayedDay);
        final int startDayOfTheWeek = calendar.get(Calendar.DAY_OF_WEEK);
        do {
            daysOfTheWeek.add(new DayOfWeek(calendar.getTime()));
            hse.sheduleteachers.Calendar.utils.DateUtils.addDay(calendar);
    ...
    public static List<String> createWeekDayTitles(int firstDayOfWeek) {
        SimpleDateFormat sdf = new SimpleDateFormat(hse.sheduleteachers.Calendar.utils.Constants.DAY_NAME_FORMAT, Locale.getDefault());
    ...
    public static List<Month> createInitialMonths(SettingsManager settingsManager) {
        ...
        for (int i = 0; i < SettingsManager.DEFAULT_MONTH_COUNT; i++) {
            ...
            hse.sheduleteachers.Calendar.utils.DateUtils.addMonth(calendar);
        }
        return months;
    }
